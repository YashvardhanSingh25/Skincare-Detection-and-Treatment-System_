    ...
    def analyze(self, day1_paths, current_paths):
        ...
        try:
            ...
            if hist_sims:
                ...
                avg_sim = float((avg_sim * 0.4) + (avg_hist_sim * 0.6))
        except Exception as e:
            print("Histogram error:", e)
        
        # 6. Ensemble Prediction
        # Use MAX instead of MEAN to capture localized issues (e.g. acne on one cheek)
        max_pred_matrix = np.max(preds_curr, axis=0)
        
        # Normalize to keep it a valid probability distribution
        avg_pred_matrix = max_pred_matrix / np.sum(max_pred_matrix)
        
        idx = np.argmax(avg_pred_matrix)
        conf = avg_pred_matrix[idx]
        
        # Calculate Closeness to Normal using embeddings similarity
        embs_curr_norm = tf.nn.l2_normalize(embs_curr, axis=1)
        scores_curr = tf.matmul(embs_curr_norm, self.embs_norm, transpose_b=True)
        closeness_list = tf.reduce_mean(scores_curr, axis=1).numpy()
        avg_closeness = float(max(0.0, np.mean(closeness_list)))
        
        return {
            "class_index": int(idx),
            "class_name": CATEGORIES[idx],
            "confidence": float(conf),
            "health_score_new": avg_h2,
            "health_score_old": avg_h1,
            "improvement": avg_h2 - avg_h1,
            "similarity": avg_sim,
            "closeness_to_normal": avg_closeness,
    ...
    def analyze_body_part(self, day1_path, current_path):
        ...
        try:
            ...
            if img1 is not None and img2 is not None:
                ...
                # Combine Pearson correlation with color histogram correlation (weigh histogram heavily for skin)
                sim = float((sim * 0.4) + (max(0.0, hist_sim) * 0.6))
        except Exception as e:
            print("Histogram error:", e)
        ...
        embs_curr_norm = tf.nn.l2_normalize(embs_curr, axis=1)
        scores_curr = tf.matmul(embs_curr_norm, self.embs_norm, transpose_b=True)
        avg_closeness = float(max(0.0, tf.reduce_mean(scores_curr).numpy()))
