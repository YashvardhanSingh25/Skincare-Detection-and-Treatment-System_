...
import { Camera, RefreshCcw, ShieldCheck, Info, Scan as ScanIcon, ArrowRight, Sparkles, CameraOff, Sun, Moon, SplitSquareHorizontal, CheckCircle2 } from 'lucide-react';
...
const Scan = () => {
  ...
  const generateSchedule = async () => {
    ...
    try {
      const scheduleRes = await fetch('http://localhost:5001/api/schedule', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          scan_image: scanMode === 'face' ? faceImages.middle : previewImage,
          age: age
        })
      });
      ...
      if (scheduleData.success) {
        ...
        alert("Failed to generate schedule tracker.");
      }
    } catch (err) {
        ...
        {!scanComplete ? (
          <div className="space-y-10">
            <div className="max-w-xl mx-auto">
              <h1 className="text-4xl font-bold text-gray-900 mb-4">Precision Skin Analysis</h1>
              <p className="text-gray-500 text-lg">Provide your details and scan your face for a personalized skincare report.</p>
            </div>

            {/* Selection Inputs */}
            <div className="max-w-md mx-auto grid grid-cols-2 gap-4">
              <div className="text-left">
                <label className="block text-sm font-bold text-gray-700 mb-2 ml-2">Skin Type</label>
                <select
                  value={skinType}
                  onChange={(e) => setSkinType(e.target.value)}
                  className="w-full bg-white border border-gray-200 px-4 py-3.5 rounded-2xl font-medium focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all cursor-pointer shadow-sm"
                >
              ...
              <div className="text-left">
                <label className="block text-sm font-bold text-gray-700 mb-2 ml-2">Age Range</label>
                <select
                  value={age}
          ...
          >
            <div className="bg-white p-12 rounded-[40px] shadow-xl border border-teal-50">
              ...
              {/* Routine Display directly on page */}
              {result?.closeness_to_normal > 95 ? (
                <div className="bg-green-50 border border-green-200 p-8 rounded-[32px] text-center mb-10">
                  <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4">
                    <CheckCircle2 size={32} />
                  </div>
                  <h3 className="text-2xl font-bold text-green-800 mb-2">You are cured! 🎉</h3>
                  <p className="text-green-700 font-medium">Your skin health score is excellent ({result.closeness_to_normal}%). You are cured and no need medical care. Keep up your good habits!</p>
                </div>
              ) : (
                result?.recommended_routine && result.recommended_routine.Morning && result.recommended_routine.Morning.length > 0 && (
                  <div className="text-left mb-10 space-y-6">
                    <h3 className="text-xl font-bold text-gray-900 border-b border-gray-100 pb-4">Recommended Routine</h3>
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="bg-orange-50 p-6 rounded-3xl border border-orange-100/50">
                        <h4 className="font-bold text-orange-600 mb-4 flex items-center gap-2"><Sun size={20} /> Morning Routine</h4>
                        <ul className="space-y-4">
                          {result.recommended_routine.Morning.map((step, i) => (
                            <li key={i} className="text-sm text-gray-700 flex items-start gap-3">
                              <span className="w-6 h-6 rounded-full bg-white text-orange-600 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 shadow-sm">{i + 1}</span>
                      ...
                      <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100/50">
                        <h4 className="font-bold text-indigo-600 mb-4 flex items-center gap-2"><Moon size={20} /> Night Routine</h4>
                        <ul className="space-y-4">
                          {result.recommended_routine.Night?.map((step, i) => (
                            <li key={i} className="text-sm text-gray-700 flex items-start gap-3">
                              <span className="w-6 h-6 rounded-full bg-white text-indigo-600 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 shadow-sm">{i + 1}</span>
                              <span className="leading-relaxed">{step.replace(/^\d+\.\s*/, '')}</span>
                            </li>
                          ))}
                        </ul>
              ...
              <button
                ...
                className="w-full py-5 bg-teal-600 text-white rounded-[24px] font-bold hover:bg-teal-700 transition-all shadow-xl shadow-teal-600/20 flex items-center justify-center gap-3 group disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isGenerating ? (
                  <>
                    <RefreshCcw className="animate-spin" size={20} />
